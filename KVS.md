...
(ie IRIS Global) named *jsdbKvs*,
with a subscript of *demo*:


        var doc = jsdb.use('jsdbKvs', 'demo')

Next, enable it to behave as a *Key/Value Store*:

        doc.enable_kvs()

Now you're ready to follow the examples described below.
...
In the *viewer* application, you'll see this has created:
...
In the *viewer* application, you'll see:
...
In the *viewer* application, you'll see that the ^jsdbKvs Global is now empty.
...
In the *viewer* application you'll now see the following:
...
In the *viewer* application you'll now see:
